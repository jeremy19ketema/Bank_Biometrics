...
export default function LoginPage() {
  ...
  const [bioSubStatus, setBioSubStatus] = useState(
    "Do not remove your finger from the scanner."
  );
  ...
  const [feed, setFeed] = useState<
    { loc: string; res: string; time: string }[]
  >([]);
  ...
  // ---- Refs ----
  const bioInterval = useRef<ReturnType<typeof setInterval> | null>(null);
  const redirectInterval = useRef<ReturnType<typeof setInterval> | null>(null);
  ...
  useEffect(() => {
    ...
    const addFeedItem = () => {
      const [loc, res] =
        feedTemplates[
          feedIdx.current % feedTemplates.length
        ];
  ...
  const handleSubmit = async (
    ...
    try {
      ...
      if (res.ok && data.success && data.user) {
        ...
        // HR page reads this from localStorage.
        // ======================================================
        if (data.token) {
          localStorage.setItem("token", data.token);
        }

        // Some backends return accessToken instead of token.
        if (data.accessToken) {
  ...
  const startBiometricScan = () => {
    ...
    bioInterval.current = setInterval(() => {
      ...
      if (pct < 0.15) {
        ...
        setBioStatus(
          "Capturing ridge pattern…"
        );
      } else if (pct < 0.75) {
        setBioStatus(
          "Cross-referencing enrollment record…"
        );
      } else if (pct < 0.95) {
        setBioStatus(
          "Matching confidence rising…"
        );
      } else {
        setBioStatus(
  ...
  const startRedirect = () => {
    ...
    setSessionToken(generatedSession);
    ...
    redirectInterval.current = setInterval(() => {
      ...
      if (count <= 0) {
        ...
        const user = JSON.parse(
          sessionStorage.getItem("aegis_user") ||
            "{}"
        );
      ...
      <div className="left">
        ...
        <div className="hero">
          ...
          <p>
            ...
            for high-value banking operations
            across 142 branches. Access is logged,
            timestamped, and reconciled to the second.
          </p>
        ...
        <div className="form-box">
          ...
          {step === 2 && (
            <div className="step active">
              ...
              <div className="panel-sub">
                Hold still — matching against
                your enrolled biometric reference.
              </div>
              ...
              <div className="bio-stage">
                ...
                <div className="bio-status mono">
                  {bioStatus}
                </div>

                <div className="bio-substatus">
                  {bioSubStatus}
                </div>
