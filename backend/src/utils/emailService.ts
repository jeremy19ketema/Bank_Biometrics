...
export async function sendPasswordResetEmail(to: string, username: string, tempPassword: string) {
  const smtpHost = process.env.SMTP_HOST;
  const smtpPort = process.env.SMTP_PORT ? parseInt(process.env.SMTP_PORT) : undefined;
  const smtpUser = process.env.SMTP_USER;
  const smtpPass = process.env.SMTP_PASS;
  ...
  const htmlContent = `
    ...
    <head>
      <meta charset="UTF-8">
      <meta name="viewport" content="width=device-width, initial-scale=1.0">
      <title>Aegis Biometrics - Password Reset</title>
    </head>
    <body style="margin: 0; padding: 0; font-family: 'Helvetica Neue', Helvetica, Arial, sans-serif; background-color: #0F1B2B; color: #EDE7D9; -webkit-font-smoothing: antialiased;">
      <table width="100%" border="0" cellspacing="0" cellpadding="0" style="background-color: #0F1B2B; padding: 40px 20px;">
        <tr>
            ...
            <table width="100%" max-width="600" border="0" cellspacing="0" cellpadding="0" style="max-width: 600px; background-color: #16233A; border: 1px solid rgba(198, 154, 76, 0.2); border-radius: 12px; box-shadow: 0 20px 60px rgba(0,0,0,0.4); overflow: hidden;">
              ...
              <tr>
                <td align="center" style="padding: 40px 40px 30px; border-bottom: 1px solid rgba(198, 154, 76, 0.1);">
                  ...
                  <p style="margin: 8px 0 0; font-size: 11px; letter-spacing: 4px; color: #C9C2AE; text-transform: uppercase; font-family: monospace;">Security Alert</p>
                </td>
              </tr>
              
              <tr>
                <td style="padding: 40px;">
                  <h2 style="margin: 0 0 20px; font-size: 22px; font-weight: 600; color: #EDE7D9;">Credentials Reset.</h2>
                  <p style="margin: 0 0 10px; font-size: 15px; line-height: 1.6; color: #C9C2AE;">Hello ${username},</p>
                  <p style="margin: 0 0 30px; font-size: 15px; line-height: 1.6; color: #C9C2AE;">An administrator has successfully reset the credentials for your account on the Aegis Biometrics Banking platform. Your new secure access credentials are listed below.</p>
                  
                  <table width="100%" border="0" cellspacing="0" cellpadding="0" style="background-color: #0B121E; border: 1px solid rgba(237, 231, 217, 0.1); border-radius: 8px; margin-bottom: 30px;">
                    <tr>
                      <td style="padding: 24px;">
                        <p style="margin: 0 0 16px; font-size: 12px; letter-spacing: 2px; color: #C69A4C; text-transform: uppercase; font-family: monospace; font-weight: bold;">[ NEW SYSTEM CREDENTIALS ]</p>
                        ...
                        <table width="100%" border="0" cellspacing="0" cellpadding="0">
                          ...
                          <tr>
                            <td width="35%" style="padding: 12px 0; color: #C9C2AE; font-size: 13px; font-family: monospace; letter-spacing: 1px;">NEW PASSCODE</td>
                            <td width="65%" style="padding: 12px 0;">
                              <span style="display: inline-block; background-color: rgba(198, 154, 76, 0.15); color: #C69A4C; padding: 6px 12px; border-radius: 4px; font-family: monospace; font-size: 16px; font-weight: 700; letter-spacing: 2px;">${tempPassword}</span>
                            </td>
                          </tr>
                  ...
                  <p style="margin: 0 0 30px; font-size: 14px; line-height: 1.6; color: #C9C2AE; font-style: italic;">
                    <strong style="color: #C69A4C;">Action Required:</strong> For security compliance, you must authenticate and change this temporary passcode immediately upon your next login.
                  </p>
  ...
  const info = await transporter.sendMail({
    from: '"Aegis System" <e0868843@gmail.com>',
    to,
    subject: "Aegis Bank - Password Reset Notification",
    html: htmlContent,
    text: `Security Alert\n\nAn administrator has successfully reset your credentials.\n\nYour New Credentials:\nUsername: ${username}\nTemporary Password: ${tempPassword}\n\nPlease log in and change your password immediately.`,
  });
...
export async function sendPasswordChangedEmail(to: string, username: string) {
  ...
  const htmlContent = `
    ...
    <head>
      <meta charset="UTF-8">
      <meta name="viewport" content="width=device-width, initial-scale=1.0">
      <title>Aegis Biometrics - Security Alert</title>
    </head>
    <body style="margin: 0; padding: 0; font-family: 'Helvetica Neue', Helvetica, Arial, sans-serif; background-color: #0F1B2B; color: #EDE7D9; -webkit-font-smoothing: antialiased;">
      <table width="100%" border="0" cellspacing="0" cellpadding="0" style="background-color: #0F1B2B; padding: 40px 20px;">
        <tr>
          <td align="center">
            <table width="100%" max-width="600" border="0" cellspacing="0" cellpadding="0" style="max-width: 600px; background-color: #16233A; border: 1px solid rgba(198, 154, 76, 0.2); border-radius: 12px; box-shadow: 0 20px 60px rgba(0,0,0,0.4); overflow: hidden;">
              ...
              <tr>
                <td style="padding: 40px;">
                  <h2 style="margin: 0 0 20px; font-size: 22px; font-weight: 600; color: #EDE7D9;">Password Changed Successfully.</h2>
                  <p style="margin: 0 0 10px; font-size: 15px; line-height: 1.6; color: #C9C2AE;">Hello ${username},</p>
                  <p style="margin: 0 0 30px; font-size: 15px; line-height: 1.6; color: #C9C2AE;">This is an automated notification to confirm that the password for your Aegis Biometrics Banking account has been successfully changed.</p>
                  
                  <p style="margin: 0 0 30px; font-size: 14px; line-height: 1.6; color: #C9C2AE; font-style: italic;">
                    <strong style="color: #C69A4C;">Action Required:</strong> If you did not authorize this change, please contact your IT Administrator immediately to secure your account.
                  </p>
                </td>
              </tr>
  ...
  const info = await transporter.sendMail({
    from: '"Aegis System" <e0868843@gmail.com>',
    to,
    subject: "Aegis Bank - Password Changed",
    html: htmlContent,
    text: `Security Alert\n\nThe password for your account has been successfully changed.\n\nIf you did not authorize this change, please contact IT immediately.`,
  });

  if (!smtpUser || !smtpPass) {
    console.log("Password changed email sent! Preview URL: %s", nodemailer.getTestMessageUrl(info));
  }
}
